...
const std::string relationName1 = "relB";
...
const int   large_relationSize = 800000;
...
void test4(){
    ...
    createRelationForward1();
	indexTests1();
	deleteRelation();
	std::cout << "test4 (stress test) passed" << std::endl;
}
...
void intTests()
{
  ...
  BTreeIndex index(relationName, intIndexName, bufMgr, offsetof(tuple,i), INTEGER);
	...
	checkPassFail(intScan(&index,4998,GT,6000,LT), 1)
	checkPassFail(intScan(&index,4999,GT,5001,LT), 0)
	checkPassFail(intScan(&index,2333,GT,2333,LT), 0)
	checkPassFail(intScan(&index,-12,GT,-11,LT), 0)
	checkPassFail(intScan(&index,0,GT,5000,LT), 4999)
	checkPassFail(intScan(&index,-1,GT,5001,LT), 5000)
}
...
void createRelationForward1()
{
  ...
  // destroy any old copies of relation file
   ...
   try{
     if (File::exists(relationName1)){
        file1 = new PageFile(relationName1, false);
        return; 
    }
   }catch(FileNotFoundException e){}

    file1 = new PageFile(relationName1, true);
  ...
  // Insert a bunch of tuples into the relation.
  for(int i = 0; i < large_relationSize; i++ )
	{
    sprintf(record1.s, "%05d string record", i);
    record1.i = i;
    record1.d = (double)i;
...
{
  std::cout << "Create a B+ Tree index on the integer field" << std::endl;
  BTreeIndex index(relationName1, intIndexName, bufMgr, offsetof(tuple,i), INTEGER);
	...
	checkPassFail(intScan(&index,-12,GT,-11,LT), 0)
	checkPassFail(intScan(&index,0,GT,5000,LT), 4999)
	checkPassFail(intScan(&index,-1,GT,35001,LT), 35001)
}
