...
function documentReadyInit() {
  ...
  jQuery('form.searchform').on('submit', function( e ){
    ...
    if (currURL.indexOf("shop") > 0){
        var a = $("#in-shop-search").val();
        var b = $("#in-shop-sear").val();
        var queryURL = "shop-search?search=" + a + "&sid=" + b;
        window.location.replace(queryURL);
        return;
    }  
    var queryURL = "query?query=" + $form.find('input').first().val(); 
    window.location.replace(queryURL);
    return;

    //$('input[name=search]').val("aaaaaaaaaa");
    //$searchModal.append('<div class="searchform-respond">'+$form.find('input').first().val()+'</div>');
    //sending form data to PHP server if fields are not empty
    //var request = $form.serialize();
    //var ajax = jQuery.post( "query", request )
    //.done(function( data ) {
  ...
  if (jQuery().slider) {
   // var a = 100
   // var b = 300;
   // if ( $(".prev_slider_min").length){
   //     a = $(".prev_slider_min").val();
   //     b = $(".prev_slider_max").val(); 
   // }
    jQuery( ".slider-range-price" ).slider({
      ...
      create: function(){
        ...
        $(".slider-range-price").slider("option", "values", [a,b])
      },
