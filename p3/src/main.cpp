///@author Augmented by Cornard Chen, Jack Chen, Yudong Huang
...
RecordId rid[num],rid1, rid2, rid3;
...
void test2()
{
    ...
    for (i = 0; i < num/3; i++) 
    {
        ...
        bufMgr->readPage(file1ptr, pid[i], page);
        sprintf((char*)tmpbuf, "test.1 Page %u %7.1f", pid[i], (float)pid[i]);
        if(strncmp(page->getRecord(rid1).c_str(), tmpbuf, strlen(tmpbuf)) != 0)
        {
            PRINT_ERROR("ERROR :: CONTENTS DID NOT MATCH");
        }
...
void test6()
{
    ...
    //bufMgr->printSelf();
    std::cout << "Test 6 starts" << std::endl;
    for (i = 1; i < num; i++) {
        bufMgr->readPage(file1ptr, pid[i], page);
        //std::cout<<i<<std::endl;
    }
...
void test8() 
{
    ...
    try {
        for (i = 0; i < num+1; i++) {
            ...
            sprintf((char*)tmpbuf, "test.3 Page %d %7.1f", pid[i], (float)pid[i]);
            rid[i] = page->insertRecord(tmpbuf);
        }
        PRINT_ERROR("ERROR :: No more frames left for allocation. Exception should have been thrown before execution reaches this point.");
    }
...
void test10()
{
    ...
    for (i = 0; i < num/2; i++){
        bufMgr->unPinPage(file1ptr, pid[i], false);
        bufMgr->unPinPage(file2ptr, pid[num-1-i], false);
        bufMgr->unPinPage(file1ptr, pid[i], false);
        bufMgr->unPinPage(file2ptr, pid[num-1-i], false);
    }
...
void test11()
{
    ...
    for(i = 0; i < num; i++)
    {
        sprintf((char*)tmpbuf, "orospu Page %u %7.1f", pid[i], (float)pid[i]);
        bufMgr->readPage(file1ptr, pid[i], page);
    ...
    {
        bufMgr->readPage(file1ptr, pid[i], page);
        sprintf((char*)&tmpbuf, "orospu Page %d %7.1f", pid[i], (float)pid[i]);
        if (strncmp(page->getRecord(rid[i]).c_str(), tmpbuf, strlen(tmpbuf)) != 0)
        {
            PRINT_ERROR("ERROR :: CONTENTS DID NOT MATCH not write back");
        }
    }
    ...
    for(i = 0; i < num; i++)
    {
        ...
        if (strncmp(page->getRecord(rid[i]).c_str(), tmpbuf, strlen(tmpbuf)) != 0)
        {
            PRINT_ERROR("ERROR :: CONTENTS DID NOT MATCH write back");
        }
    }
...
void test12(){
    std::cout << "test 12 starts" << std::endl;
    bufMgr->readPage(file1ptr, pid[1], page);
    Page tmp = *page;
    bufMgr->disposePage(file1ptr, page->page_number());
    try{
        bufMgr->readPage(file1ptr, pid[1], page);
        //exception should happen when reading a disposed page
        std::cout << "test 12 failed" << std::endl;
    } catch (InvalidPageException) {
        std::cout << "test 12 passed" << std::endl;
    }
    //resumes the empty slot with another page
    bufMgr->allocPage(file1ptr, pid[1], page);
    sprintf((char*)tmpbuf, "test.1 Page %u %7.1f", pid[1], (float)pid[1]);
    rid[1] = page->insertRecord(tmpbuf);
...
void test13(){
    ...
    std::cout << "test 13 starts" << std::endl;
    PageId mypid[200];
    RecordId myrid[200];   
    //fill the buffer fully with new pages
    for (int i = 0; i < 100; i++){
        bufMgr->allocPage(file7ptr, mypid[i], page);
        sprintf((char*)tmpbuf, "test.7 Page %u %7.1f", mypid[i], (float)mypid[i]);
        myrid[i] = page->insertRecord(tmpbuf);
        //bufMgr->unPinPage(file7ptr, mypid[i], true);
    }
    //unpin all pages
    for (int i = 0; i < 100; i ++) {
        bufMgr->unPinPage(file7ptr, mypid[i], true);
    }
    //fill the buffer fully with other new pages
    ...
    for (int i = 0; i < 100; i ++) {
        try
        {
            //std::cout << "pid: " << mypid[i] << std::endl;
            bufMgr->unPinPage(file7ptr, mypid[i], false);
        }catch(PageNotPinnedException e) {
            count ++;
        }
    }
    if (count == 0) {
        ...
        //the former 100 pages should be removed from buffer,
        //so unpinPage should not report any exception
        std::cout << "test 13 failed." << std::endl;
        std::cout << "expected 0, got: " << count << std::endl; 
        bufMgr->printSelf();
    }
}
